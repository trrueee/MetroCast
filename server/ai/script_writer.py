...
# ---------------------------------------------------------------------------
# Duration estimation (Chinese TTS: ~4.5 chars/sec)
# ---------------------------------------------------------------------------

CHARS_PER_SECOND = 4.5

# Target total character count for 8-15 min episodes
TARGET_MIN_CHARS = 2000
TARGET_MAX_CHARS = 4000
MIN_ACCEPTABLE_CHARS = 1200
...
class PodcastEpisodeScript(BaseModel):
    ...
    summary: str = ""
    fullScript: str = ""                      # the complete flowing script (source of truth)
    segments: List[Segment] = Field(default_factory=list)
    sources: List[Source] = Field(default_factory=list)

    @property
    def full_script(self) -> str:
        """The complete spoken script as one continuous text."""
        if self.fullScript:
    ...
    def total_chars(self) -> int:
        """Total characters across all segment texts."""
        return sum(len(seg.text) for seg in self.segments)

    @property
    def estimated_total_duration_sec(self) -> float:
        """Estimate total spoken duration from character count."""
        speech_sec = self.total_chars / CHARS_PER_SECOND
        pause_sec = sum(seg.pauseAfterMs for seg in self.segments) / 1000.0
        return speech_sec + pause_sec
...
def validate_episode_script(script: PodcastEpisodeScript) -> ValidationResult:
    ...
    # W3. Total character count out of target range
    total_chars = script.total_chars
    if total_chars < MIN_ACCEPTABLE_CHARS:
        result.warnings.append(
            f"Total script length {total_chars} chars is below minimum "
            f"acceptable {MIN_ACCEPTABLE_CHARS} chars (~5 min)"
        )
    elif total_chars < TARGET_MIN_CHARS:
        result.warnings.append(
            f"Total script length {total_chars} chars is below target "
            f"range ({TARGET_MIN_CHARS}-{TARGET_MAX_CHARS}) — estimated "
            f"duration {script.estimated_total_duration_sec:.0f}s"
        )
    elif total_chars > TARGET_MAX_CHARS:
        result.warnings.append(
            f"Total script length {total_chars} chars exceeds target "
            f"range ({TARGET_MIN_CHARS}-{TARGET_MAX_CHARS}) — estimated "
            f"duration {script.estimated_total_duration_sec:.0f}s"
        )

    # W4. fullScript must equal concatenation of segment texts
    if script.fullScript:
        joined = "".join(seg.text for seg in script.segments)
        if script.fullScript != joined:
            deviation = abs(len(script.fullScript) - len(joined))
            result.warnings.append(
                f"fullScript ({len(script.fullScript)} chars) does not match "
                f"segments concatenation ({len(joined)} chars, diff={deviation}). "
                f"Script may be fragmented instead of flowing."
            )
...
**这是最重要的要求。你必须先在心里完整写完一整篇播客稿——从头到尾读起来像一个人在一次录音中自然说出的话。然后再把它按段落类型切分到 segments 中。**

不要在脑子里把每个段落当成独立的"填空题"来写。先写出 fullScript（完整口播文本），确保它是一篇流畅、连贯、有人情味的完整播客稿，再从中提取每个 segment 的 text。

**如何判断你写的是"完整稿"还是"拼接稿"：**
- 完整稿：去掉 segmentId、type 这些标签后，把每段的 text 连起来读，像一个人一气呵成说完的
- 拼接稿：每段各自独立，段与段之间没有自然衔接，换段时有明显的"话题跳转感"
...
**fullScript 总字数必须在 2000-4000 字之间。这是硬性要求，低于 2000 字视为不合格。**
- 2000-3000 字 ≈ 8-12 分钟（含停顿），是最佳时长区间。
- 不要因为素材少就缩短节目。素材不足时，在 city_story 和 commute_tip 中加入更多生活化细节、具体场景描述、实用建议，用自然的口语展开。
- 如果素材过多，优先保留高风险线路变化和安全提醒，精简城市小事。
- 绝对不要低于 1200 字（约 5 分钟），低于此值意味着内容严重不足。
- 输出前务必数一下 fullScript 的总字数，确认在 2000 字以上再提交。

各段落字数参考（保持弹性）：
- opening: 80-150 字
- headline: 200-500 字
- line_update: 200-500 字（无素材可跳过）
- commute_tip: 200-500 字
- city_story: 300-700 字
- safety: 150-400 字
- ending: 80-150 字
...

{
  ...
  "fullScript": "完整口播文本，这是小站姐从开场到结尾一气呵成说出的全部内容。必须是一篇读起来流畅自然的完整稿子。把 segments 中每段的 text 按顺序连起来就应该等于 fullScript。",
  "segments": [
    {
      "segmentId": "seg_001",
      "type": "opening",
      "title": "段落小标题（内部使用，不口播）",
      "text": "该段落的完整口播文本（必须和 fullScript 中对应部分完全一致）",
...
在输出 JSON 之前，请确认：
1. fullScript 从头到尾读起来是否像一个人在自然说话，而不是拼接的？
2. 把所有 segment text 按顺序连起来，是否正好等于 fullScript（逐字一致）？
3. 是否只有第一个 segment 是 opening？
4. 是否只有最后一个 segment 是 ending？
5. 段与段之间的过渡是否自然，而不是生硬的话题跳转？
6. 高风险 segment 是否都有 sourceIds？
7. 有没有"据悉""重磅""震撼"等违禁词？
8. fullScript 总字数是否在 2000-4000 字之间？
...
**最重要的要求：先写 fullScript（完整口播文本），确保它是一篇从头到尾一气呵成的完整稿子，然后再从中提取每个 segment 的 text。** 不要反过来——不要先分别填写每个 segment 再拼成 fullScript。那样写出来的稿子会有"拼接感"。

要求：
1. fullScript 必须是一篇流畅、连贯、有温度的完整口播稿，不是几个独立段落的拼接
2. 把所有 segment text 按顺序连起来，必须正好等于 fullScript（逐字一致）
3. 严格遵循段落类型顺序和长度要求
4. 绝对禁止在非 ending 段落出现结束语
5. 绝对禁止在非 opening 段落自我介绍
6. 语言自然，像一个熟悉城市交通的朋友在陪听众通勤
7. 高风险信息必须标注 riskLevel="high" 并提供来源
...
class ScriptWriter:
    ...
    def generate_script(
        ...
        logger.info(
            ...
            "%d total chars, ~%.0fs estimated duration",
            script.title, len(script.segments), len(script.sources),
            script.total_chars, script.estimated_total_duration_sec,
        )
        return script
        ...
        try:
            ...
            for i, s in enumerate(raw.get("sources") or []):
                if not isinstance(s, dict):
                    continue
                sid = s.get("sourceId") or f"src_{i + 1:03d}"
                sources.append(Source(
                    sourceId=sid,
                    title=s.get("title") or "",
                    url=s.get("url") or "",
                    type=s.get("type") or "rss",
                ))
            ...
            for i, seg in enumerate(raw.get("segments") or []):
                if not isinstance(seg, dict):
                    continue
                sid = seg.get("segmentId") or f"seg_{i + 1:03d}"
                seg_type = seg.get("type") or "headline"
                ...
                # Robust: handle null/None for list fields (AI sometimes returns null)
                src_ids = seg.get("sourceIds") or []
                ...
                segments.append(Segment(
                    ...
                    estimatedDurationSec=seg.get("estimatedDurationSec") or 0,
                    pauseAfterMs=seg.get("pauseAfterMs") or 800,
                    sourceIds=src_ids,
                    riskLevel=risk,
                ))

            # fullScript: use AI-provided value, or derive from segments as fallback
            full_script = raw.get("fullScript", "")
            if not full_script:
                full_script = "".join(seg.text for seg in segments)
                logger.warning("fullScript not provided by AI, derived from segments")
